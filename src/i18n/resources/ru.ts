	...
	settings: {
		...
		taskProperties: {
			...
			titleCard: {
				...
				occurrenceFilenameTemplate: "Шаблон имени файла экземпляра",
				occurrenceFilenameTemplateHelp:
					"Шаблон имени файла для материализованных экземпляров повторяющихся задач. Оставьте поле пустым, чтобы сохранить существующее имя (заголовок родительской задачи с числовым суффиксом). Явно выберите детализацию с помощью {{occurrenceDate}}, {{occurrenceWeek}}, {{occurrenceMonth}}, {{occurrenceYear}} или {{occurrenceMonthName}}. Также доступны все обычные переменные имени файла. Родительская задача может переопределить шаблон через указанное ниже свойство frontmatter.",
				occurrenceFilenameProperty: "Свойство переопределения шаблона экземпляра",
				occurrenceFilenamePropertyHelp:
					"Имя свойства frontmatter родительской повторяющейся задачи, которое переопределяет шаблон имени файла её экземпляров."
			},
			tagsCard: {
		...
		integrations: {
			...
			httpApi: {
				...
				authToken: {
					...
					description: "Токен TaskNotes, а не API-ключ вашего поставщика ИИ. Чтобы создать токен, включите HTTP API, оставьте это поле пустым и перезапустите Obsidian. Затем скопируйте созданный токен в настройки Bearer-аутентификации клиента. Обновляйте настройки клиентов при каждом изменении токена.",
